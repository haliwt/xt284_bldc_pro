...
void NO_HallSensor_DectorPhase(void)
{
       ...
       static uint8_t u =0,v=0,w=0;
        C1CON2 = 0x00; //比较控制寄存器2 --
		C1CON0 = 0x80; //比较控制寄存器0 --enable compare
		delay_us(20);//delay_us(20);
		if(intBEMF&0x80){w == 0x01;} //w --BEMF
		C1CON0 = 0x81;
		delay_us(20);
		if(intBEMF&0x80){v == 0x01;} //V ---BEMF
		C1CON0 = 0x82;
		delay_us(20);
		if(intBEMF&0x80){u == 0x01;}//u ---BEMF 
    ...
    switch(gPhase){
        ...
        case 0:  
             ...
             if(u ==1){  // BEMF "B"  //换向到下一个位置 Ev=C1P1
...
void CCW_MotorRun(void)
{
   static uint8_t u =0,v=0,w=0;
    ...
    switch(gPhase){
        ...
        case 0:  
                //AB- "2" --查看“Ev”  AB ->AC->BC->BA->CA->CB->AB(循环)
               MOS_A_H;    //换向到 位置 6  A+ B-   from BC to AC
              
             
             if(w ==0){  // BEMF "B"  //换向到下一个位置 Ev=C1P1
